 ...
 *
 * This will show one/two columns full width post and the rest will arrange itself in two columns
 */


if( ! class_exists('Purea_Fashion_Posts_Layout_2_Widget')) :

class Purea_Fashion_Posts_Layout_2_Widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			'purea_fashion_posts_layout_2_widget', // Base ID
			esc_html__( 'Purea Fashion: Posts Layout 2 Widget', 'purea-fashion' ), // Name
			array( 'description' => esc_html__( 'This will show one/two columns full width post and the rest will arrange itself in two columns. ', 'purea-fashion'), ) // Args
		);		     
	}
	...
	public function widget( $args, $instance ) {
		...
		$no_of_columns = ! empty( $instance['no_of_columns'] ) ? absint($instance['no_of_columns']) : 2;
		...
		<div class="pf-posts-layout-2-wrapper" style="margin-top:<?php echo $spacing_top; ?>px;margin-bottom:<?php echo $spacing_bottom; ?>px; ">
			<div class="pf-posts-layout-2-lists-wrapper">
						...
						<?php
							...
							if($query-> have_posts()) : $postCount = 0;
								while( $query-> have_posts() ) : $postCount++; $query->the_post(); ?>
									<div id="post-<?php the_ID(); ?>" class="pf-layout-2-category-post category-post column-<?php echo $no_of_columns; ?>">
										<div class="pf-layout-2-category-post-content">
											<div class="pf-section-layout-2-area-box">
												<div class="col-count">
													<div class="col-first">
														<div class="pf-layout-2-post-image">
															<?php
																if(has_post_thumbnail()) { 
																	...
																	?><img src="<?php echo $post_img_url; ?>" alt="<?php esc_attr_e('post-image','purea-fashion'); ?>" /><?php
																}
															?>
														</div>
													</div>
													<div class="col-second">
														<div class="pf-layout-2-area-content">
	...
	public function form( $instance ) {
	    ...
	    ?>     	  	    	
			...
			<p>
				<label>
                    <input type="radio" value="1" name="<?php echo $this->get_field_name( 'no_of_columns' ); ?>" <?php checked( $no_of_columns, '1' ); ?> id="<?php echo $this->get_field_id( 'no_of_columns' ); ?>" />
                    <?php esc_attr_e( '1 Column', 'purea-fashion' ); ?>
                </label>
                <label>
...
if( ! function_exists('purea_fashion_register_posts_layout_2_widget')) :
// register widget
function purea_fashion_register_posts_layout_2_widget() {
    register_widget( 'Purea_Fashion_Posts_Layout_2_Widget' );
}
endif;

add_action( 'widgets_init', 'purea_fashion_register_posts_layout_2_widget' );
